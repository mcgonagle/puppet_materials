...
class product::mh::tools inherits product::mh {
  package{"dos2unix": ensure => latest }

  file { "/tmp/v4nicator.sh":
    source => "puppet:///modules/product/mh/tools/v4nicator.sh",
    owner => "root", group => "root", mode => "0700", }
  
  file { "/tmp/cleanerator.sh":
    source => "puppet:///modules/product/mh/tools/cleanerator.sh",
    owner => "root", group => "root", mode => "0700", }

  file { "/tmp/stagerator.sh":
    source => "puppet:///modules/product/mh/tools/stagerator.sh",
    owner => "root", group => "root", mode => "0700",}

  file { "/tmp/appymlizer.sh":
    source => "puppet:///modules/product/mh/tools/appymlizer.sh",
    owner => "root", group => "root", mode => "0700",}

  file { "/tmp/rollbackerator.sh":
    source => "puppet:///modules/product/mh/tools/rollbackerator.sh",
    owner => "root", group => "root", mode => "0700",}
