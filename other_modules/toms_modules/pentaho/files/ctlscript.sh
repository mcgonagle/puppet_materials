#!/bin/sh
#
# biserver        Startup script for the pentaho biserver
#
# chkconfig: - 85 15
# description: biserver is a business intelligence server.
...
TOMCAT_SCRIPT=$INSTALLDIR/server/biserver-ee/tomcat/scripts/ctl.sh
...
THIRD_SCRIPT=$INSTALLDIR/server/data-integration-server/scripts/ctl.sh
POSTGRESQL_SCRIPT=$INSTALLDIR/postgresql/scripts/ctl.sh
NAGIOS_SCRIPT=$INSTALLDIR/nagios/scripts/ctl.sh
JETTY_SCRIPT=$INSTALLDIR/server/enterprise-console/scripts/ctl.sh
...
help() {
	...
	if test -x $THIRD_SCRIPT; then	
	    echo "       $0 (start|stop|restart|status) data-integration-server"
	fi
...
if [ "x$1" = "xhelp" ] || [ "x$1" = "x" ]; then
    ...
    elif [ "x$2" = "xdata-integration-server" ]; then
        if test -x $THIRD_SCRIPT; then	
            $THIRD_SCRIPT start  
            THIRD_ERROR=$?
        fi
