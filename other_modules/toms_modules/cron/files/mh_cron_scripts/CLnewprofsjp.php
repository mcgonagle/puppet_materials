...
$filename="newprofiles$end_date".".csv";
...
<H1>Weekly New Profiles Statistics</H1>
...
$jpNewState.= "New Profiles by state MANHUNT.jp week ending\n$input_end_date\n";
foreach ($States_en as $id=>$statename) {
if ($queryNewJpState) { $queryNewJpState.=" UNION "; }
  $queryNewJpState.="SELECT '$States_en[$id]',COUNT(uid) AS cnt
  FROM Users
  WHERE status NOT IN('delete','hold') AND created<'$end_date_time6' AND
  created>=DATE_ADD(DATE_SUB('$end_date',INTERVAL 6 DAY),INTERVAL 6 HOUR)
  AND state='$id'";
}

$jpNewCtry.= "New Profiles by country MANHUNT.jp week ending\n$input_end_date\n";
$queryNewJp="SELECT 'jp', COUNT(uid) FROM Users
WHERE status NOT IN('delete','hold')
AND created<='$end_date_time6' AND created>DATE_SUB('$end_date_time6',interval 7 day);";

#################### Total Profiles

$jpTotCtry.= "Total Profiles by country MANHUNT.jp week ending\n$input_end_date\n";
$queryTotalJp="SELECT 'jp', COUNT(uid) FROM Users
WHERE status NOT IN('delete','hold') AND created<'$end_date_time6';";

$jpTotState.= "Total Profiles by state MANHUNT.jp week ending\n$input_end_date\n";
foreach ($States_en as $id=>$statename) {
if ($queryTotJpState) { $queryTotJpState.=" UNION "; }
  $queryTotJpState.="SELECT '$States_en[$id]',COUNT(uid) AS cnt
  FROM Users
  WHERE status NOT IN('delete','hold') AND created<'$end_date_time6' 
  AND state='$id'";
}
...
$db='jp';
$jpNewState.=display_vert($queryNewJpState,$db);
$db='jp';
$jpTotState.=display_vert($queryTotJpState,$db);
$db='jp';
$jpTotCtry.=display_vert($queryTotalJp,$db);
$db='jp';
$jpNewCtry.=display_vert($queryNewJp,$db);
